...
import (
	...
	"github.com/lonegunmanb/terraform-alicloud-schema/schema"
	"golang.org/x/oauth2"
)
...
func refreshSchema() *version.Version {
	...
	if err != nil {
		log.Fatalf("Failed to refresh AliCloud schema: %v", err)
	}
	return v
}
...
func checkGitHubTag(schemaVersion string) (bool, error) {
	...
	for {
		tags, resp, err := client.Repositories.ListTags(context.Background(), "lonegunmanb", "terraform-alicloud-schema", options)
		if err != nil {
			return false, err
		}
